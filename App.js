      ...
      <UserProvider>
        <Layout>
          <Routes>
            ...
            <Route path="/signup/" element={authCtx.isLoggedIn ? <Navigate to="/1/1" /> : <CreateAccountForm />} />
            <Route path="/login/*" element={authCtx.isLoggedIn ? <Navigate to="/1/1" /> : <AuthPage />} />
            <Route path="/profile/" element={!authCtx.isLoggedIn ? <Navigate to="/1/1" /> : <ProfilePage />} />

            {/* club */}
            <Route path="/club" element={<ClubLayout />}>
              <Route path="clubpage/:pageId" element={<ClubListPage />} />
              <Route path=":clubId" element={<ClubOnePage />} />
              <Route path="createclub" element={authCtx.isLoggedIn ? <CreateClubPage /> : <Navigate to="/1/1" />} />
              <Route
                path="updateclub/:clubId"
                element={authCtx.isLoggedIn ? <UpdateClubPage /> : <Navigate to="/1/1" />}
              />

              {/* gallery */}
              <Route path="createGallery/:clubId" element={<CreateGalleryPage />} />
              <Route
                path="updateGallery/:clubId/:galleryId"
                element={authCtx.isLoggedIn ? <UpdateGalleryPage /> : <Navigate to="/1/1" />}
              />
              <Route path="updategallerycomment/:galleryId/:id" element={<UpdateGalleryCommentPage />} />
              <Route path=":clubId/gallery/page/:pageId" element={<GalleryListPage />} />
              <Route path=":clubId/gallery/:galleryId" element={<GalleryOnePage />} />

              {/* article */}
              <Route
                path="createarticle/:clubId"
                element={authCtx.isLoggedIn ? <CreateArticlePage /> : <Navigate to="/1/1" />}
              />
              <Route
                path="updatearticle/:clubId/:articleId"
                element={authCtx.isLoggedIn ? <UpdateArticlePage /> : <Navigate to="/1/1" />}
              />
              <Route path="updatecomment/:articleId/:commentId" element={<UpdateCommentPage />} />
              <Route path=":clubId/article/page/:pageId" element={<ArticleListPage />} />
              <Route path=":clubId/article/:articleId" element={<ArticleOnePage />} />
